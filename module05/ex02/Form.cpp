...
// Constructors
Form::Form()
: _name("default"), _isSigned(false), _signGrade(150), _executeGrade(150), _target("Default target")
{
	std::cout << "Default Constructor called of Form" << std::endl;
}

Form::Form(const Form &copy)
: _name(copy.getName()), _isSigned(false), _signGrade(copy.getSignGrade()), _executeGrade(copy.getExecuteGrade()), _target(copy.getTarget())
{
...
: _name(name), _isSigned(false), _signGrade(signGrade), _executeGrade(executeGrade), _target(target)
{
